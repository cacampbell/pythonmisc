...
class SAM2BAM:
    ...
    def make_commands(self, sam_files):
        sys.stdout.write("Formatting Commands...\n")
        cmds = []

        for lane_file in sam_files:
            bamfile = os.path.basename(lane_file).replace(".sam", ".bam")
            output_file = os.path.join(os.path.split(lane_file)[0], bamfile)
            cmd = "samtools view -Sb -o %s -@ %s %s" \
